...
public class SkullHandler {
	 ...
	 * 
	 * public static SkullHandler getInstance() { return instance; }
	 * 
	 * private Method asBukkitCopy;
	 * 
	 * @Getter private Method asNMSCopy;
	 * 
	 * @SuppressWarnings("rawtypes") private Class craftItemStack;
	 * 
	 * @SuppressWarnings("rawtypes") private Class itemStack;
	 * 
	 * @Getter private ConcurrentHashMap<String, Object> skulls = new
	 ...
	 * 
	 * private void add(String playerName) { if (!skullsToLoad.contains(playerName)
	 * && !getSkulls().containsKey(playerName)) { skullsToLoad.add(playerName); } }
	 * 
	 ...
	 * 
	 * 
	 * @SuppressWarnings("deprecation") public org.bukkit.inventory.ItemStack
	 * getItemStack(String playerName) throws IllegalAccessException,
	 * IllegalArgumentException, InvocationTargetException { if
	 * (hasSkull(playerName)) { return (ItemStack) asBukkitCopy.invoke(null,
	 * skulls.get(playerName)); } else { loadSkull(playerName); } return new
	 * ItemBuilder("PLAYER_HEAD").setSkullOwner(playerName).toItemStack(); }
	 * 
	 * @SuppressWarnings("deprecation") public ItemStack getSkull(String playerName)
	 * { return new
	 * ItemBuilder("PLAYER_HEAD").setSkullOwner(playerName).toItemStack();
	 * 
	 * }
	 * 
	 * public boolean hasSkull(String playerName) { if (playerName != null &&
	 * skulls.containsKey(playerName)) { if (skulls.get(playerName) != null) {
	 * return true; } } return false; }
	 * 
	 * @SuppressWarnings({ "unchecked", "deprecation" }) public void load() { try {
	 * 
	 * gameProfile =
	 ...
	 * 
	 * 
	 * craftItemStack = ReflectionUtils.getClassForName( "org.bukkit.craftbukkit." +
	 * NMSManager.getInstance().getVersion() + "inventory.CraftItemStack");
	 * 
	 * if (NMSManager.getInstance().isVersion("1.12", "1.13", "1.14", "1.15",
	 * "1.16")) { itemStack = NMSManager.getInstance().getNMSClass("ItemStack"); }
	 * else { itemStack =
	 * ReflectionUtils.getClassForName("net.minecraft.world.item.ItemStack"); }
	 * 
	 * asNMSCopy = craftItemStack.getDeclaredMethod("asNMSCopy", ItemStack.class);
	 * asNMSCopy.setAccessible(true); } catch (SecurityException |
	 * NoSuchMethodException e) { AdvancedCorePlugin.getInstance().debug(e); }
	 * 
	 * try { asBukkitCopy = craftItemStack.getDeclaredMethod("asBukkitCopy",
	 * itemStack); asBukkitCopy.setAccessible(true); } catch (SecurityException |
	 * NoSuchMethodException e) { AdvancedCorePlugin.getInstance().debug(e); }
	 * 
	 * SkullThread.getInstance().loadThread(); }
	 * 
	 * public void loadSkull(Player player) { loadSkull(player.getName()); }
	 * 
	 * public void loadSkull(final String playerName) { if
	 * (AdvancedCorePlugin.getInstance().getOptions().isLoadSkulls() &&
	 * AdvancedCorePlugin.getInstance().isEnabled()) { if
	 * (PluginUtils.getInstance().getFreeMemory() > 300 &&
	 * PluginUtils.getInstance().getMemory() > 800) { if (Bukkit.isPrimaryThread())
	 * { timer.execute(new Runnable() {
	 * 
	 * @Override public void run() { add(playerName); } }); } else {
	 * add(playerName); } } else { AdvancedCorePlugin.getInstance()
